...
wlim = 2*pi*[-70 70]; % brain range
figure(1);im(3, embed(winit_masked,mask), 'winit', wlim), cbar('Hz')
clear mag1 good
drawnow
...
[out,cost_qm,time_qm] = fmap_est_qm(winit(mask), yik_c(mask,:,:), p.etime,...
    smap_c(mask,:),'order', 1, 'l2b', -4, 'dim',3, ...
...
[out,cost_cg,time_cg] = fmap_est_pcg_ls(winit(mask), yik_c(mask,:,:),p.etime, ...
    smap_c(mask,:),'order', 1, 'l2b', -4, ...
    'niter', niter_cg,'maskR', mask,'precon','diag','gammaType','PR');
wmap_cg = embed(out.ws,mask);
figure(2);subplot(121);im(wmap_cg(:,:,:,end), 'cg', wlim)
subplot(122);semilogy(time_cg,cost_cg,'.-k')
argsError_cg_d = {'NCG-MLS-D', time_cg, wmap_cg};
%% 4. NCG implementation: ichol precon
niter_cg = 50;
[out,cost_cg,time_cg] = fmap_est_pcg_ls(winit(mask), yik_c(mask,:,:),p.etime, ...
    smap_c(mask,:),'order', 1, 'l2b', -4, ...
    'niter', niter_cg,'maskR', mask,'precon','ichol','gammaType','PR');
wmap_cg = embed(out.ws,mask);
figure(2);subplot(121);im(wmap_cg(:,:,:,end), 'cg', wlim)
subplot(122);semilogy(time_cg,cost_cg,'.-k')
argsError_cg_ichol = {'NCG-MLS-IC', time_cg, wmap_cg};
%% RMSE plots
...
scale = 1/2/pi;
clim = [-100,100];
wplot = wmap_cg(:,:,zplot,end);
figure;
subplot(511);im(['notick'], 'row',1, [], [], mask(:,:,zplot).*mag(:,:,zplot), [0,1],' ');cbar
subplot(512);im(['notick'], 'row',1, [], [], winit(:,:,zplot)*scale, clim,' ');cbar
subplot(513);im(['notick'], 'row',1, [], [], wplot*scale, clim,' ');cbar
subplot(514);im(['notick'], 'row',1, [], [], mask(:,:,zplot).*wtrue(:,:,zplot)*scale, clim,' ');cbar
subplot(515);im(['notick'], 'row',1, [], [], mask(:,:,zplot).*(wtrue(:,:,zplot)-wplot)*scale, 5*[-10,10],' ');cbar
colormap gray